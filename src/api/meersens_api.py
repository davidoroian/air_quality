    ...
    if existing_entries:
        for entry in entries:
            if entry['datetime'] > existing_entries[-1].recording_date or entry['datetime'] < existing_entries[0].recording_date:
                ...
                create_pm10_entry(entry_id=entry_id, qualification=entry['pollutants']['pm10']['index']['qualification'], description=entry['pollutants']['pm10']['index']['description'], value=entry['pollutants']['pm10']['index']['value'], date=entry['datetime'])
                create_pm25_entry(entry_id=entry_id, qualification=entry['pollutants']['pm25']['index']['qualification'], description=entry['pollutants']['pm25']['index']['description'], value=entry['pollutants']['pm25']['index']['value'], date=entry['datetime'])
                create_co_entry(entry_id=entry_id, qualification=entry['pollutants']['co']['index']['qualification'], description=entry['pollutants']['co']['index']['description'], value=entry['pollutants']['co']['index']['value'], date=entry['datetime'])
    else:
        for entry in entries:
            create_entry(entry['datetime'])
            entry_id = get_entry(entry['datetime']).id
