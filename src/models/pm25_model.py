...
def create_pm25_entry(entry_id, qualification, description, value, date):
    with get_conn() as conn:
        cursor = conn.cursor()
        cursor.execute(f"INSERT INTO pm25 (entry_id, qualification, description, value, date) VALUES (?, ?, ?, ?, ?)", entry_id, qualification, description, value, date)
        conn.commit()